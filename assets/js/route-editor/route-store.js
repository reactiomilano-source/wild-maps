(function (window) {
	...
	function isEmptyMainRoute(route) {
		...
		return route && String(route.id || '') === 'route-main' && (!route.name || String(route.name) === 'Main route') && !route.geojson && !waypoints.length;
	}
	...
	function RouteStore(initialState) {
		...
		this.routes = [];
		this.dirty = false;
		this.load(initialState || { routes: [], activeRouteId: '' });
	}
