...
class Route_Collection {
	...
	private static function empty_main_route( $route ) {
		if ( ! is_array( $route ) ) { return false; }
		$waypoints = isset( $route['waypoints'] ) && is_array( $route['waypoints'] ) ? $route['waypoints'] : [];
		return self::DEFAULT_ROUTE_ID === ( $route['id'] ?? '' ) && ( ! isset( $route['name'] ) || 'Main route' === $route['name'] ) && empty( $route['geojson'] ) && empty( $waypoints );
	}

	public static function normalize_route( $route, $index = 0 ) {
		if ( ! is_array( $route ) || self::empty_main_route( $route ) ) { return null; }
	...
	public static function payload( $project_id ) {
		...
		return [ 'active_route_id' => $active ?: ( $routes[0]['id'] ?? '' ), 'routes' => $routes ];
	}
}
